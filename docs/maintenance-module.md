...
1. она персонально назначена ЧЕЛОВЕКУ ИЗ ТЕХСЛУЖБЫ, И
2. автор подтвердил это галочкой «это задача по эксплуатации» в форме задачи.

**Кто такой «человек из техслужбы»** (`lib/maintenance/staff-server.ts`):
носитель сидовой роли `maintenance_head` / `maintenance_staff` **ЛИБО** любой,
кому выдано право `maintenance.manage` — ролью (в т.ч. СОБСТВЕННОЙ ролью
владельца) или персонально; персональный deny снимает признак.

Изначально признаком были только два сидовых кода — и это оказалось неверным:
владелец завёл своего человека под собственной ролью («מנהל תחזוקה»), признак не
сработал, галочка в форме не появилась, и задача до доски не дошла, ничего при
этом не сообщив. Признак «настроен работать в модуле» берётся из настроек, а не
из списка в коде, поэтому переименования и собственные роли больше не ломают
связку. Проверяются РЕАЛЬНЫЕ строки прав, а не `hasMaintenancePrivilege`: иначе
суперадминский обход сделал бы человеком из техслужбы каждого администратора.
