// ─── Единое отображение ошибок БД → HTTP для модуля «Эксплуатация» ───────────
//
...
export function mapDbError(
  ...
  switch (error.code) {
    ...
    case '23503':                    // foreign_key_violation (напр. несуществующее здание/комната)
      return { status: 400, message: 'Ссылка на несуществующую запись' }
    case '23505':                    // unique_violation
      return { status: 409, message: 'Запись уже существует' }
    case '23514':                    // check_violation (кривая категория/приоритет/статус)
