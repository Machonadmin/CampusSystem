...
// ─── Типы ─────────────────────────────────────────────────────────────────────
//
// Тот же паттерн, что lib/doctor/permissions.ts, но для module='psychologist' и
// с общим reduceScopes из lib/permissions/scope.ts. Две привилегии; данные не
// привязаны к подразделению, поэтому в MVP по факту scope='all' (любой scope без
// target трактуется как «разрешено»).
//
// ЧУВСТВИТЕЛЬНЫЕ ДАННЫЕ О ПСИХИЧЕСКОМ ЗДОРОВЬЕ: каждый маршрут API обязан
// проходить requirePsychologistPrivilege('view' | 'manage').
...
type PrivilegesMap = Partial<Record<PsychologistPrivilege, Scope>>
...
export function clearPsychologistPermissionsCache(personId?: string): void {
...
/**
 * Загружает все psychologist-привилегии для массива ролей. Если привилегия есть
...
async function loadPrivileges(roleCodes: string[]): Promise<PrivilegesMap> {
  ...
  const { data: privs, error: privsErr } = await sb
    ...
    .eq('module', 'psychologist')
    .in('role_id', roleIds)

  if (privsErr || !privs) return {}

  return reduceScopes<PsychologistPrivilege>(privs)
}
...
async function getUserAccess(session: SessionPayload): Promise<CacheEntry> {
  ...
  const [rolePrivileges, personGrants] = await Promise.all([
    ...
    loadPersonModuleGrants('psychologist', session.person_id),
  ])
  const privileges = applyPersonGrants<PsychologistPrivilege>(rolePrivileges, personGrants)
...
/**
 * Проверяет, есть ли у пользователя привилегия модуля «Психолог».
 * Данные не привязаны к подразделению, поэтому любой присутствующий scope
 * означает «разрешено».
 */
export async function hasPsychologistPrivilege(
  session: SessionPayload | null,
  privilege: PsychologistPrivilege,
): Promise<boolean> {
...
export async function getPsychologistPrivilegeScope(
  session: SessionPayload | null,
  privilege: PsychologistPrivilege,
): Promise<Scope | null> {
 ...
 *
 * Пример:
 *   const session = await requirePsychologistPrivilege('view')
 */
export async function requirePsychologistPrivilege(
  privilege: PsychologistPrivilege,
  ...
  const ok = await hasPsychologistPrivilege(session, privilege)
  if (!ok) {
