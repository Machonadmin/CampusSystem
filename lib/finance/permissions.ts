...
async function getUserAccess(session: SessionPayload): Promise<CacheEntry> {
  ...
  const [rolePrivileges, personGrants] = await Promise.all([
    ...
    loadPersonModuleGrants('finance', session.person_id),
  ])
  const privileges = applyPersonGrants<FinancePrivilege>(rolePrivileges, personGrants)
...
export async function hasFinancePrivilege(
  ...
  // superadmin (кроме студенческого principal — портальная изоляция) — всё.
