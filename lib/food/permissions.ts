...
// ─── Типы ─────────────────────────────────────────────────────────────────────
//
// Тот же паттерн, что lib/dormitory/permissions.ts, но для module='food' и с
...
type PrivilegesMap = Partial<Record<FoodPrivilege, Scope>>
...
export function clearFoodPermissionsCache(personId?: string): void {
...
/**
 * Загружает все food-привилегии для массива ролей. Если привилегия есть у
...
async function loadPrivileges(roleCodes: string[]): Promise<PrivilegesMap> {
  ...
  return reduceScopes<FoodPrivilege>(privs)
}
...
async function getUserAccess(session: SessionPayload): Promise<CacheEntry> {
  ...
  const [rolePrivileges, personGrants] = await Promise.all([
    ...
    loadPersonModuleGrants('food', session.person_id),
  ])
  const privileges = applyPersonGrants<FoodPrivilege>(rolePrivileges, personGrants)
...
/**
 * Проверяет, есть ли у пользователя привилегия модуля «Питание».
 * Данные не привязаны к подразделению, поэтому любой присутствующий scope
 * означает «разрешено».
 */
export async function hasFoodPrivilege(
  session: SessionPayload | null,
  privilege: FoodPrivilege,
): Promise<boolean> {
...
export async function getFoodPrivilegeScope(
  session: SessionPayload | null,
  privilege: FoodPrivilege,
): Promise<Scope | null> {
 ...
 *
 * Пример:
 *   const session = await requireFoodPrivilege('view')
 */
export async function requireFoodPrivilege(
  privilege: FoodPrivilege,
  ...
  const ok = await hasFoodPrivilege(session, privilege)
  if (!ok) {
