...
// ─── Типы ─────────────────────────────────────────────────────────────────────
//
// Тот же паттерн, что lib/contacts/permissions.ts и lib/finance/permissions.ts,
// но для module='sponsors' и с общим reduceScopes из lib/permissions/scope.ts.
// Две привилегии; справочник доноров не привязан к подразделению, поэтому в MVP
// по факту scope='all' (любой scope без target трактуется как «разрешено»).
//
// Каждый маршрут API обязан проходить requireSponsorsPrivilege('view'|'manage').
...
type PrivilegesMap = Partial<Record<SponsorsPrivilege, Scope>>
...
export function clearSponsorsPermissionsCache(personId?: string): void {
...
/**
 * Загружает все sponsors-привилегии для массива ролей. Если привилегия есть у
...
async function loadPrivileges(roleCodes: string[]): Promise<PrivilegesMap> {
  ...
  return reduceScopes<SponsorsPrivilege>(privs)
}
...
/**
 * Проверяет, есть ли у пользователя привилегия модуля «Спонсоры».
 * Справочник доноров не привязан к подразделению, поэтому любой присутствующий
 * scope означает «разрешено».
 */
export async function hasSponsorsPrivilege(
  ...
  // 'own' здесь бессмыслен и НЕ должен по ошибке открыть все данные спонсоров.
  return scope !== 'own'
}

/** Получить scope, с которым у пользователя есть привилегия. null — если нет. */
export async function getSponsorsPrivilegeScope(
  session: SessionPayload | null,
  privilege: SponsorsPrivilege,
): Promise<Scope | null> {
 ...
 */
export async function requireSponsorsPrivilege(
  privilege: SponsorsPrivilege,
  ...
  const ok = await hasSponsorsPrivilege(session, privilege)
  if (!ok) {
