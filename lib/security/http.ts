...
// ─── Единое отображение ошибок БД → HTTP для модуля «Безопасность» ───────────
//
...
export function mapDbError(
  ...
  switch (error.code) {
    ...
    case '23503':                    // foreign_key_violation (напр. несуществующее здание)
      return { status: 400, message: serverT('invalid_reference') }
    case '23505':                    // unique_violation
      return { status: 409, message: serverT('record_exists') }
    case '23514':                    // check_violation (кривая категория/серьёзность/статус)
