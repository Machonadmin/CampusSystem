...
// ─── Типы ─────────────────────────────────────────────────────────────────────
//
// Тот же паттерн, что lib/finance/permissions.ts и lib/psychologist/permissions.ts,
// но для module='reports'. «Отчёты» — READ-ONLY дашборд руководства; данные не
// привязаны к подразделению, поэтому любой присутствующий scope означает
// «разрешено». Департамент-скоуп сохранён для совместимости с общей моделью прав.
...
type PrivilegesMap = Partial<Record<ReportsPrivilege, Scope>>
...
export function clearReportsPermissionsCache(personId?: string): void {
...
/**
 * Загружает все reports-привилегии для массива ролей.
...
async function loadPrivileges(roleCodes: string[]): Promise<PrivilegesMap> {
  ...
  return reduceScopes<ReportsPrivilege>(privs)
}
...
async function getUserAccess(session: SessionPayload): Promise<CacheEntry> {
  ...
  const [rolePrivileges, personGrants] = await Promise.all([
    ...
    loadPersonModuleGrants('reports', session.person_id),
  ])
  const privileges = applyPersonGrants<ReportsPrivilege>(rolePrivileges, personGrants)
...
/**
 * Проверяет, имеет ли пользователь привилегию модуля «Отчёты».
 * Данные не привязаны к подразделению, поэтому любой присутствующий scope
 * означает «разрешено».
 */
export async function hasReportsPrivilege(
  session: SessionPayload | null,
  privilege: ReportsPrivilege,
): Promise<boolean> {
 ...
 */
export async function getReportsPrivilegeScope(
  session: SessionPayload | null,
  privilege: ReportsPrivilege,
): Promise<Scope | null> {
 ...
 *
 * Пример:
 *   const session = await requireReportsPrivilege('view')
 */
export async function requireReportsPrivilege(
  privilege: ReportsPrivilege,
  ...
  const ok = await hasReportsPrivilege(session, privilege)
  if (!ok) {
