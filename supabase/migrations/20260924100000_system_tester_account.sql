...
--
-- ЧТО НЕ ВЫДАЁТСЯ (сознательно):
--   • права с risk = 'critical' — медицина (doctor), эмоциональная консультация
--     (psychologist), бирур яхадут (jewishness), чувствительные личные данные
--     (persons.view_sensitive), чувствительные тревоги, настройки системы и
--     «אבטחת מידע»;
--   • выгрузки (*export*) — массовый вынос данных, даже если это «чтение»;
--   • устаревшие коды (is_legacy).
--   Нужно шире — владелец выдаёт точечно в «אבטחת מידע» или меняет фильтр ниже.
--
...
BEGIN
  ...
  SELECT v_person_id, mp.module, mp.privilege_code, TRUE,
   ...
   WHERE mp.level IN ('access', 'view')
     AND mp.risk <> 'critical'
     AND mp.is_legacy = FALSE
     AND mp.privilege_code NOT LIKE '%export%';
