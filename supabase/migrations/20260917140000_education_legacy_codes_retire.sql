...
--
-- ─── Об предупреждении Supabase при запуске ─────────────────────────────────
--
-- SQL Editor покажет два предупреждения, и ответ на них РАЗНЫЙ:
--
--   «destructive operations» — верно и намеренно: миграция удаляет строки.
--
--   «creates tables without enabling RLS» — про _edu_map и _edu_before. Это
--   ВРЕМЕННЫЕ таблицы (CREATE TEMP TABLE): они живут только внутри этого
--   запуска и только в этом соединении, ни anon, ни authenticated их не видят,
--   и последние строки файла их удаляют. Защищать там нечего.
--   Нажимать нужно «Run without RLS». «Run and enable RLS» попытается
--   выполнить ALTER TABLE ... ENABLE ROW LEVEL SECURITY по временным таблицам
--   и может оборвать запуск на середине.
--
--   (Не путать с 20260916130000: там создавались НАСТОЯЩИЕ постоянные таблицы,
--   и там RLS был обязателен.)
--
