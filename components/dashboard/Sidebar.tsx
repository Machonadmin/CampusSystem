...
const I = {
  ...
  doctor: 'M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z M12 9v6m-3-3h6',
...
export default function Sidebar() {
  ...
  const itemClass = (href: string) =>
    `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
      isActive(href)
        ? 'bg-[#2D3170] text-white'
        : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'
    }`

  return (
    <aside
      className="fixed top-16 bottom-0 z-40 w-56 flex flex-col bg-white border-e border-gray-200 overflow-y-auto"
      style={{ [isRTL ? 'right' : 'left']: 0 }}
    >
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        {/* Top links */}
        {TOP_ITEMS.map(item => (
          <Link key={item.key} href={item.href} className={itemClass(item.href)}>
