...
import type { LeadViewData } from '../../education/leads/[id]/LeadViewClient'
import type { StatusHistoryEntry } from '@/components/education/StudentLifecyclePanel'
import type { AlumniProfileData } from '@/components/alumni/AlumniProfilePanel'
...
export default async function AlumniViewPage({ params }: Props) {
  ...
  // Право на просмотр модуля выпускников
  ...
  // Карточка выпускника — только для выпускников. Иначе назад в список.
  if (j.education_status !== 'graduated') {
  ...
  const [{ data: interests }, { data: communities }, { data: relatives }, { data: history }, { data: alumniProfile }] =
    await Promise.all([
      ...
      sb.from('alumni_profiles')
        .select('id, graduation_year, institution, direction, current_location, current_occupation, notes')
        .eq('person_id', j.person_id)
        .maybeSingle(),
    ])
  ...
  const profile: AlumniProfileData | null = alumniProfile
    ? {
        id: alumniProfile.id,
        graduation_year: alumniProfile.graduation_year,
        institution: alumniProfile.institution,
        direction: alumniProfile.direction,
        current_location: alumniProfile.current_location,
        current_occupation: alumniProfile.current_occupation,
        notes: alumniProfile.notes,
      }
    : null
