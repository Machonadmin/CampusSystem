...
// ── Module icon paths (Heroicons outline 24px) ───────────────────────────────
...
// Per-module accent colours (icon bg + ring)
const COLORS: Record<string, { bg: string; icon: string }> = {
  education:   { bg: 'bg-blue-50',   icon: 'text-blue-600' },
  finance:     { bg: 'bg-emerald-50',icon: 'text-emerald-600' },
  dormitory:   { bg: 'bg-violet-50', icon: 'text-violet-600' },
  food:        { bg: 'bg-orange-50', icon: 'text-orange-500' },
  maintenance: { bg: 'bg-yellow-50', icon: 'text-yellow-600' },
  security:    { bg: 'bg-red-50',    icon: 'text-red-600' },
  alumni:      { bg: 'bg-indigo-50', icon: 'text-indigo-600' },
  sponsors:    { bg: 'bg-pink-50',   icon: 'text-pink-600' },
  doctor:      { bg: 'bg-teal-50',   icon: 'text-teal-600' },
  psychologist:{ bg: 'bg-purple-50', icon: 'text-purple-600' },
  documents:   { bg: 'bg-sky-50',    icon: 'text-sky-600' },
  reports:     { bg: 'bg-lime-50',   icon: 'text-lime-600' },
  contacts:    { bg: 'bg-cyan-50',   icon: 'text-cyan-600' },
  settings:    { bg: 'bg-gray-100',  icon: 'text-gray-600' },
}

const MODULES = [
  'education', 'finance', 'dormitory', 'food',
  'maintenance', 'security', 'alumni', 'sponsors',
  'doctor', 'psychologist', 'documents', 'reports',
  'contacts', 'settings',
] as const
...
function ModuleIcon({ moduleKey }: { moduleKey: string }) {
  ...
  const color = COLORS[moduleKey] ?? { bg: 'bg-gray-100', icon: 'text-gray-500' }
  return (
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #2D3170 0%, #3d4494 60%, #4BAED4 100%)' }}
      >
        {/* Decorative circles */}
        <span
