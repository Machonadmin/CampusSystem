      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 16, marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div>
            ...
            <select aria-label={t('rollover.month')} disabled={FROZEN} style={inp} value={settings.rollover_month} onChange={e => setSettings({ ...settings, rollover_month: Number(e.target.value) })}>
              {Array.from({ length: 12 }, (_, i) => i + 1).map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
          <div>
            <label style={lbl}>{t('rollover.day')}</label>
            <select aria-label={t('rollover.day')} disabled={FROZEN} style={inp} value={settings.rollover_day} onChange={e => setSettings({ ...settings, rollover_day: Number(e.target.value) })}>
