...
export default function GradeEntryPanel({ assessment, canSetGrades, accentColor, onClose, onSaved }: Props) {
  ...
  const inputStyle: React.CSSProperties = {
    padding: '5px 8px', fontSize: 13, border: '1px solid #D1D5DB', borderRadius: 6,
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', marginTop: 12, borderRadius: 8, border: '1px solid #E5E7EB' }}>
          {loading ? (
            <div style={{ padding: 24, textAlign: 'center', color: '#9CA3AF', fontSize: 13 }}>{t('entry_loading')}</div>
          ) : error ? (
            <div style={{ padding: 24, textAlign: 'center', color: '#DC2626', fontSize: 13 }}>{error}</div>
          ) : students.length === 0 ? (
            <div style={{ padding: 24, textAlign: 'center', color: '#9CA3AF', fontSize: 13 }}>{t('entry_empty')}</div>
          ) : (
            students.map((s, i) => (
              <div
                key={s.journey_id}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <input
                      ...
                      style={{ ...inputStyle, width: 70, textAlign: 'center', background: canSetGrades ? '#fff' : '#F9FAFB' }}
                    />
                    <span style={{ fontSize: 12, color: '#9CA3AF', whiteSpace: 'nowrap' }}>/ {formatScore(maxScore)}</span>
                  </div>
                  <input
                    ...
                    style={{ ...inputStyle, width: 180, background: canSetGrades ? '#fff' : '#F9FAFB' }}
                  />
                </div>
              </div>
            ))
