...
/**
 * POST /api/sponsors/[id]/person-link — решение ответственного (sponsors.manage)
 * по связи донора с центральной персоной (решение №11).
...
export async function POST(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const res = await applyPersonLinkAction(sb, 'sponsors', params.id, body.action, {
      canManage: true,
      canViewPersons: await canViewPersonsSafe(session),
    })
    if ('view' in res) return NextResponse.json({ person_link: res.view })
    if (res.error === 'not_found') return apiError('donor_not_found', 404)
