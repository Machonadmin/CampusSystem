...
import { openVisitCountsByJourney, allergyJourneyIds } from '@/lib/doctor/visits-server'

/**
 * GET /api/doctor/students — студенты (education_journeys status='student') с
 *   persons, числом ОТКРЫТЫХ приёмов и флагом аллергии (заполнено поле allergies
 *   в медкарте). Право: doctor.view. Фильтр ?search= — app-side по ФИО/email/
 *   телефонам (как в других модулях). ЧУВСТВИТЕЛЬНЫЕ ДАННЫЕ — только под view.
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const openCounts = await openVisitCountsByJourney(sb, journeyIds)
    const allergyFlags = await allergyJourneyIds(sb, journeyIds)
    ...
    let students = journeys.map(j => {
      ...
      return {
        ...
        open_visits: openCounts.get(j.id) ?? 0,
        has_allergies: allergyFlags.has(j.id),
      }
    })
