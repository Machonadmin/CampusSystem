 ...
 * psychologist/superadmin) может открыть документ ТОЛЬКО если его journey
 * прямо сейчас находится на активном этапе `medical` (т.е. реально направлена
 * к врачу). Это защищает от чтения произвольных документов (IDOR).
 */

const MEDICAL_SIGNER_ROLES = ['doctor', 'psychologist', 'superadmin']
