...
export async function GET() {
  try {
    ...
    const allowed = isSuper || await canDoEducationInAny(session, 'view_students')
    ...
    const links = (ct ?? []) as Array<{ teacher_id: string; class_group_id: string }>
    if (links.length === 0) return NextResponse.json({ teachers: [] })

    const teacherIds = [...new Set(links.map(l => l.teacher_id))]
    const groupIds = [...new Set(links.map(l => l.class_group_id))]
