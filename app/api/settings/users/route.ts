...
export async function GET() {
  try {
    ...
    const result = (accounts ?? []).map(acc => {
      ...
      return { ...acc, full_name: person?.full_name ?? '', photo_url: person?.photo_url ?? null, roles: userRoles }
    })
...
export async function POST(request: NextRequest) {
  try {
    await guard()
    const sb = createServerClient()
    const { full_name, login_email, password, role_ids = [] } = await request.json() as {
      full_name: string; login_email: string; password: string; role_ids?: string[]
    }

    if (!full_name || !login_email || !password)
    ...
    const { data: account, error: e2 } = await sb.from('person_accounts')
      .insert({ person_id: person.id, login_email: login_email.toLowerCase().trim(), password_hash, is_active: true, last_login: null })
      .select('id').single()
    if (e2) throw e2

    if (role_ids.length > 0) {
      const { error: e3 } = await sb.from('person_roles')
        .insert(role_ids.map(role_id => ({ person_id: person.id, role_id, assigned_by: null })))
      if (e3) throw e3
    }

    return NextResponse.json({ person_id: person.id, account_id: account.id }, { status: 201 })
  } catch (err: unknown) {
