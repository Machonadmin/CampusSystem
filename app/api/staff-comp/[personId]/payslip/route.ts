...
import { canViewStaffComp, canApprovePayslip, isSelfCompTarget, monthRange, sumEntries, canAccessStaffCompPerson } from '@/lib/finance/staff-comp'
...
export async function POST(request: NextRequest, props: { params: Promise<{ personId: string }> }) {
  ...
  try {
    ...
    if (!(await canAccessStaffCompPerson(session, params.personId, 'approve_payment'))) return apiError('forbidden', 403)
