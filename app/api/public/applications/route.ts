 ...
 * форме, боты его заполняют; (2) rate limit по IP. Затем — та же атомарная
 * бизнес-операция, что и во внутренней форме: RPC create_application (всегда
 * НОВАЯ запись — person_id не передаём) + автозапуск процесса «Набор». Актёр —
 * служебная запись SYSTEM_PERSON_ID (у публичной формы нет пользователя).
 *
 * Дополнительно создаётся задача-уведомление отделу «Администрация», чтобы
 * персонал увидел новую заявку (помимо появления лида в дашборде).
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 4. Автозапуск процесса «Набор» — best-effort (как и во внутренней форме)
    const { error: startErr } = await sb.rpc('start_process', {
      p_process_code: 'recruitment',
      p_journey_id: journeyId,
      p_actor_id: SYSTEM_PERSON_ID,
    })
    if (startErr) console.error('[public/applications] start_process:', startErr)

    // 5. Уведомление персоналу — задача отделу «Администрация».
